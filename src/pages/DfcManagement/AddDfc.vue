            ...
            <div class="p-grid p-fluid">
                <div class="p-col-12">
                    ...
                    <div v-show="render2" class="p-p-2">
                        <strong >DFC 업로드 성공 {{ success }}/{{ totalrecord }}</strong>
                    </div>
            ...
            if (isInvalid) {
                ...
                return axios.post('/admin/dfc/add', this.formData).then((data) => {
                    if (data.data.data.errorArray.length == 0) {
                        ...
                        this.totalrecord = data.data.data.totalRecord;
                        if (this.success == 0) {
                            this.$toast.add({ severity: 'error', summary: '거절되었습니다', detail: '등록된 정보가 없습니다.', life: 3000 });
                        } else {
                            this.$toast.add({ severity: 'info', summary: '확인', detail: `${this.success} 명의 DFC가 업로드 되었습니다.`, life: 3000 });
                        }
                    }
                });
            }
