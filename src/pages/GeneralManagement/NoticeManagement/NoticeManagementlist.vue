            ...
            <div class="card">
                <div class="p-d-flex p-jc-between p-mb-2">
                    ...
                    <div>
                        ...
                        <router-link to="/GeneralManagement/NoticeManagement/NoticeManagementadd">
                ...
                <DataTable :value="customer1" :paginator="false" class="p-datatable-gridlines" :rows="10" dataKey="id" :rowHover="true" :loading="loading1" :filters="filters1" responsiveLayout="scroll">
                    ...
                    <Column header="내용" style="min-width: 12rem">
                        <template #body="{ data }">
                            <span class="p-column-title">Description</span>
                            <template v-if="data.description.length < 8"> {{ data.description }}</template>
                            <template v-else> {{ data.description.substring(0, 8) + '..' }}</template>
                            <!-- {{ data.description }} -->
                        </template>
                    </Column>

                    <Column header="링크 텍스트" style="min-width: 12rem">
                        <template #body="{ data }">
                            <span class="p-column-title">Link Text</span>
                            <template v-if="data.link_text.length < 8">
                                <span v-if="data.link_text != null">{{ data.link_text }}</span>
                                <span v-else> - </span>
                            </template>
                            <template v-else> {{ data.link_text.substring(0, 8) + '..' }}</template>
                        </template>
                    </Column>

                    <Column header="URL" style="min-width: 12rem">
                        <template #body="{ data }">
                            <span class="p-column-title">Url</span>
                            <template v-if="data.url.length < 8">
                                <span v-if="data.url != null">{{ data.url }}</span>
                                <span v-else> - </span>
                            </template>
                            <template v-else> {{ data.url.substring(0, 8) + '..' }}</template>
                    ...
                    <Column header="배너 쇼" style="min-width: 12rem">
                        <template #body="{ index, data }">
                            <span class="p-column-title">Banner show</span>
                            <InputSwitch :modelValue="getStatus(data.show_in_banner)" @click="Changestatus(index, data.id)" />
                        </template>
                    </Column>
                    ...
                    <Column header="기능">
                        <template #body="{ data }">
                            ...
                            <div style="display: flex">
                                <router-link :to="'/GeneralManagement/NoticeManagement/NoticeManagementedit/' + data.id"
                                    ><Button label="info" class="p-button2"><i class="pi pi-user-edit p-mr-2"></i> </Button>
                                </router-link>
                                <router-link :to="'/GeneralManagement/NoticeManagement/NoticeManagementview/' + data.id"
...
export default {
    ...
    mounted() {
        ...
        this.storageData = localStorage.getItem('NoticeManagement') ? JSON.parse(localStorage.getItem('NoticeManagement')) : '';
    
        console.log(this.storageData, 'data');

        if (this.storageData != '') {

            this.noticeManagementlistpagination = this.storageData.page;
        ...
        if (this.noticeManagementlistpagination == '' || this.noticeManagementlistpagination == null) {
            ...
            this.noticeManagementlist(searchListParams);
        } else {
            const searchListParams = {
                page: this.pagination.current_page,
                limit: this.pagination.pagination_limit,
            ...
            const searchLocalParams = {
                page: Number(this.noticeManagementlistpagination),
                limit: this.pagination.pagination_limit,
                title: this.title,
                startDate: this.startDate,
                endDate: this.endDate,
            ...
            this.noticeManagementlist(searchLocalParams);
        }
    ...
    methods: {
        ...
        Changestatus(index, id) {
            ...
            console.log(this.customer1[index].notice_image);
            if (this.customer1[index].notice_image == '' || this.customer1[index].notice_image == null) {
                this.$toast.add({ severity: 'info', summary: 'error', detail: '활성화를 하려면 이미지 업로드가 필요합니다.', life: 3000 });
                this.noticeManagementlist();
            } else {
                // var statustext = this.customer1[index].show_in_banner;
                // this.customer1[index].show_in_banner = statustext === 'active' ? 'inactive' : 'active';
                var complexstatus = { ...{ id: id } };
                // console.log('companystatus', companystatus);
                this.noticeManagementService.complexchange_banner(complexstatus).then((res) => {
                    console.log(res);
                    if (res.status === 200) {
                        if (res.data.success == false) {
                            this.$toast.add({ severity: 'error', summary: '확인됨', detail: res.data.message, life: 3000 });
                            this.noticeManagementlist({ page: 1, limit: this.pagination.pagination_limit });
                        } else {
                            this.$toast.add({ severity: 'info', summary: '확인됨', detail: '공지사항 관리자 이미지가 성공적으로 업데이트되었습니다.', life: 3000 });
                            this.noticeManagementlist({ page: 1, limit: this.pagination.pagination_limit });
                        }
                    }
                });
        ...
        paginate(page) {
            ...
            this.noticeManagementService
                .getNoticeManagementList({ page: page, limit: this.pagination.pagination_limit, title: this.title, startDate: this.searchdate, endDate: this.searchenddate })
                .then((res) => {
                    this.customer1 = res.data.result;
                    this.totalrecords = res.data.total_records;
        ...
        resetUser() {
            localStorage.setItem('NoticeManagement', '');
            (this.title = ''), (this.startDate = ''), (this.endDate = ''), this.noticeManagementlist({ page: 1, limit: this.pagination.pagination_limit });
        },
        searchuser() {
            ...
            if (this.title !==' ' || this.startDate !==' ' || this.endDate !==' ' ) {
                ...
                this.customer1 = true;
                this.noticeManagementService
                    .getNoticeManagementList(searchParams)
                    .then((res) => {
                        console.log('res', res);
                        this.customer1 = res.data.result;
                        this.totalrecords = res.data.total_records;
                ...
                accept: () => {
                    self.noticeManagementService
                        ...
                        .then(function (response) {
                            ...
                            self.noticeManagementlist({ page: self.pagination.current_page, limit: self.pagination.pagination_limit, title: self.title, startDate: self.searchdate, endDate: self.searchenddate });
