    ...
    <div class="card">
        ...
        <form>
            ...
            <div class="p-d-flex p-jc-end p-ai-center" style="float: left">
                <Button label="저장" iconPos="left" class="p-button p-button-outlined p-button-sm p-mr-2 p-mb-2" @click="addTag"></Button>
...
import validateCreateTag from '../../../validations/tag/validateCreateTag';
...
export default {
    ...
    methods: {
        ...
        addTag() {
            ...
            const { isInvalid, error } = validateCreateTag(vcheckData);
            if (isInvalid) {
                this.error = error;
                console.log(error);
            } else {
                ...
                return axios
                    .post('/admin/tags/add', { name: this.name, status: this.status == '' ? 'inactive' : 'active', color: this.color })
                    .then(() => {
                        // alert('Tag added successfully.');
                        this.$router.push({ name: 'TagManagementlist' });
                        setTimeout(() => {
                            this.$toast.add({ severity: 'info', summary: '확인.', detail: '태그가 성공적으로 추가되었습니다.', life: 3000 });
                        }, 1000);
                    })
                    .catch((error) => {
                        this.$toast.add({ severity: 'error', summary: '확인', detail: error.response.data.message, life: 3000 });
                    });
            }
        },
    },
